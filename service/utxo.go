...
import (
	...
	"os"
	"os/signal"
	"pld_bitcoin_service/utils/bitcoin/bech32"
	"pld_bitcoin_service/utils/bitcoin/btcleveldb"
	"pld_bitcoin_service/utils/bitcoin/keys"
	"strconv"
	"strings"
	"syscall"
	"unsafe"
...
var chainstate = flag.String("chainstate", "F://Bitcoinbak//testnet3//chainstate", "Location of bitcoin chainstate db.")          // chainstate folder
var addrindex = flag.String("addrindex", "F://Bitcoinbak//testnet3//chainstate//addrindex", "Location of bitcoin chainstate db.") // chainstate folder

/**
遍历chainstate,并将结果放入新的addrindex db
仅在启动适合运行就一次就可以了
*/
func LoadUTXO() {
	//fields := flag.String("f", "count,txid,vout,height,amount,coinbase,nsize,script,type,address", "Fields to include in output. [count,txid,vout,height,amount,coinbase,nsize,script,type,address]")
	testnetflag := flag.Bool("testnet", true, "Is the chainstate leveldb for testnet?") // true/false
	...
	//遍历数据
	addrindexDB, err := leveldb.OpenFile(*addrindex, nil)
	if err != nil {
		log.Panic(err)
	}
	defer addrindexDB.Close()
	...
	for iter.Next() {
		...
		if prefix == 67 { // 67 = 0x43 = C = "utxo"
			...
			if fieldsSelected["txid"] {
				...
				utxo.Txid = hex.EncodeToString(txid)
			}
			...
			if fieldsSelected["height"] || fieldsSelected["coinbase"] || fieldsSelected["amount"] || fieldsSelected["nsize"] || fieldsSelected["script"] || fieldsSelected["type"] || fieldsSelected["address"] {
				...
				utxo.Nsize = fmt.Sprintf("%d", nsize)
				...
				if fieldsSelected["script"] {
					...
					utxo.Script = hex.EncodeToString(script)
				}
			...
			//写入
			//Len := unsafe.Sizeof(*utxo)
			//testBytes := &SliceMock{
			//	addr: uintptr(unsafe.Pointer(utxo)),
			//	cap:  int(Len),
			//	len:  int(Len),
			//}
			//data := *(*[]byte)(unsafe.Pointer(testBytes))
			//这一步我去其他地方做就会乱码
			//var utxoValue *Utxo = *(**Utxo)(unsafe.Pointer((&data)))
			//log.Printf("utxoValue :%+v ", utxoValue)

			var result bytes.Buffer
			encoder := gob.NewEncoder(&result)
			encoder.Encode(utxo)
			utxoBytes := result.Bytes()

			keys := []byte(utxo.Address + "_" + strconv.Itoa(i))
			err = addrindexDB.Put(keys, utxoBytes, nil)
			//log.Print("keys:::" + bytes2str(keys))
			if err != nil {
				log.Panic("leveldb insert fail", keys)
			}
			//查询是否写入
			//list := list.New()
			//iter := addrindexDB.NewIterator(util.BytesPrefix([]byte(utxo.address)), nil)
			//for iter.Next() {
			//	value := iter.Value()
			//	var utxoValue *Utxo = *(**Utxo)(unsafe.Pointer((&value)))
			//	log.Printf("k:[%s]\nv:[%+v]\n", iter.Key(), utxoValue)
			//	list.PushBack(iter.Value())
			//}
		}
		// Increment Count
		i++
...
通过地址获取utxo
*/
func GetUtxo(addr string) ([]Utxo, error) {
	addrindexDB, err := leveldb.OpenFile(*addrindex, nil)
	if err != nil {
		log.Panic(err)
	}
	...
	iter := addrindexDB.NewIterator(util.BytesPrefix([]byte(addr)), nil)
		for iter.Next() {
		value := iter.Value()
		//var utxoValue *Utxo = *(**Utxo)(unsafe.Pointer((&value)))
...
测试单个地址查询
*/
func Getkey(addr string) {
	addrindexDB, err := leveldb.OpenFile("F://Bitcoinbak//testnet3//chainstate//addrindex", nil)
	if err != nil {
		log.Panic(err)
	}
	defer addrindexDB.Close()

	data, err := addrindexDB.Get([]byte(addr), nil)
...
func Put(addr string) {
	...
	utxo := Utxo{
		...
		Coinbase: "0",
		Nsize:    "0",
		Script:   "c90889a2a6544b31bc6dd5803765f895c4108ecaaddr",
		Types:    "p2pkh",
		Address:  "myqvLpN5vmcSuwEMV4cfRHAL88Aq5MmofQ_12",
	}
	...
	i := 1
	for i <= 2 {
		keys := []byte(addr + "_" + strconv.Itoa(i))

		err = addrindexDB.Put(keys, utxoBytes, nil)
		log.Print(bytes2str(keys))
		if err != nil {
			log.Panic(err)
		}
		i++
