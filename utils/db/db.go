...
var Datadir string = "G:\\Bitcoin\\testnet3"
...
type BlockIndexRecord struct {
	...
	NDataPos       uint32
	NUndoPos       uint32
	HashPrev       utils.Hash256
	HashMerkleRoot utils.Hash256
	NTime          time.Time
	NBits          uint32
	NNonce         uint32
}
...
func OpenIndexDb(blockchainDataDir string) (*IndexDb, error) {
	db, err := leveldb.OpenFile(blockchainDataDir+"/blocks/index/", nil)
	if err != nil {
		return nil, err
	}
...
func OpenTxIndexDb(blockchainDataDir string) (*IndexDb, error) {
	db, err := leveldb.OpenFile(blockchainDataDir+"/indexes/txindex", nil)
	if err != nil {
		return nil, err
	}
...
func GetBlockIndexRecordByBigEndianHex(indexDb *IndexDb, blockHash string) (*BlockIndexRecord, error) {
	...
	blockHashInBytes = utils.ReverseHex(blockHashInBytes)
...
func FailIfReindexing(indexDb *IndexDb) {
	result, err := GetReindexing(indexDb)
	if err != nil {
		log.Fatal(err)
	}
	if result {
...
func GetFlag(indexDb *IndexDb, name []byte) (bool, error) {
	...
	err := indexDb.Put(command, []byte("1"), nil)
	if err != nil {
		log.Panic(err)
	}
